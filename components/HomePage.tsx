...
export default function HomePage({ faq, latestPosts }: { faq: ReactNode; latestPosts: { slug: string; title: string; date: string }[] }) {
  ...
  return (
    <div className="site-refresh min-h-screen font-sans">
            ...
            {[
              ...
              <div key={i} className="group text-center">
                <div className="relative overflow-hidden rounded-2xl mb-6 shadow-md aspect-video">
                  <Image src={item.img} alt={item.title} fill sizes="(max-width: 767px) 100vw, 400px" className="object-cover transition-transform duration-500 group-hover:scale-105" />
      ...
      {latestPosts.length > 0 && (
        <section aria-labelledby="latest-reading-heading" className="home-reading py-12 bg-white">
          <div className="container max-w-5xl">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              <h2 id="latest-reading-heading" className="text-xl font-bold text-primary">雨漏りの、気になること。</h2>
              <Link href="/blog" className="inline-flex min-h-11 items-center gap-1 text-sm text-primary underline underline-offset-4">記事をすべて見る<ArrowRight aria-hidden="true" className="h-4 w-4" /></Link>
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              {latestPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="min-w-0 border-t border-slate-200 py-4 text-primary hover:text-cta">
                  <time dateTime={post.date} className="block text-xs text-slate-500 mb-2">{post.date}</time>
                  <h3 className="text-base font-medium leading-relaxed line-clamp-2">{post.title}</h3>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm text-white/80">
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>〒{BUSINESS_INFO.postalCode}<br />{BUSINESS_INFO.address}</span>
                </li>
                <li className="flex items-start gap-2">
          ...
          <div className="container py-6">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 mb-4">
              <div className="flex items-center gap-2">
                <Image src={DROCO_ICON_URL} alt="株式会社ドローン工務店" width={24} height={24} sizes="24px" className="w-6 h-6 rounded-sm object-contain" />
                <span className="text-xs text-white/50">運営：株式会社ドローン工務店</span>
              </div>
              <div className="flex items-center gap-2">
                <Image src={LIXIL_BADGE_URL} alt="LIXILリフォームネット" width={24} height={24} sizes="24px" className="w-6 h-6 rounded-sm object-contain" />
